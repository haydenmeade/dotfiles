-- src https://github.com/ray-x/go.nvim/blob/8f8ab4b968d08bd70ecda1f2909fb5e6177452ca/lua/go/utils.lua
local util = {}
local fn = vim.fn
...
function util.root_dirs()
  local dirs = {}
  local root = fn.systemlist({ "go", "env", "GOROOT" })
  table.insert(dirs, root[1])
  local paths = fn.systemlist({ "go", "env", "GOPATH" })
...
function util.interface_list(pkg)
  local p = fn.systemlist({ "go", "doc", pkg })
  util.log(p)
  local ifaces = {}
  if p then
